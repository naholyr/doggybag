...
function Agent (jobType, options, dependencies) {
    ...
    // Spread ready
    c.on('ready', function () { e.emit('ready') });
        ...
        process.nextTick(function () {
            try {
                agentModule.run(jobData, function (err, result) {
                    if (err) {
                        respond({ "error": "JOB_FAILED", "data": err.stack || err.toString() }, err.reject ? err.requeue : null, err.reject && (err.requeueDelay || options.requeueDelay));
                    } else {
